...
class SharedMarketContext:
    """Market signal computed once by the primary account and shared to all accounts in a group job.
...
class AITradingService:
    ...
    async def _run_pipeline(
        ...
        # ── 3.5 Shared-context fast path (secondary group accounts) ──────────
        import dataclasses as _dc
        _skip_analysis = shared_ctx is not None
        _built_shared_ctx: "SharedMarketContext | None" = None

        if _skip_analysis:
            # Signal already computed by primary account — skip OHLCV/LLM entirely.
            # Copy signal to prevent confidence-gate mutation from affecting other accounts.
            tf_upper = shared_ctx.timeframe
        ...
        if not _skip_analysis:
            ...
            if not rule_based:
                ...
                if news_signal and news_signal != "HOLD":
                    ...
                    if market_dir != "HOLD" and market_dir != news_signal:
                        ...
                        return AnalysisResult(signal=signal, order_placed=False, ticket=None, journal_id=0, shared_ctx=_built_shared_ctx)
            ...
            _built_shared_ctx = SharedMarketContext(
                symbol=symbol,
